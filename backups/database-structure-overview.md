...
| 网易基础库 | `database/netease-artist-base.json` | 网易原始艺人基础数据 | 39299 | `name`, `aliases`, `name_keys`, `platform_ids`, `regions`, `languages`, `artist_type` | 姓名和基础身份信息较全 |
...
| 标签表 | 静态标签主表 | 两平台 ID 表 + 后续标签抓取结果 | `artist_key` | `artist_key`, `qq_name`, `netease_name`, `gender`, `regions`, `languages`, `styles` | 低频，一年一次左右 |
| QQ 月更表 | QQ 高频变化数据表 | QQ 详情页/统计抓取 | `artist_key` | `artist_key`, `album_count`, `song_count`, `fan_count`, `updated_at` | 月更 |
| 网易月更表 | 网易高频变化数据表 | 网易详情页/统计抓取 | `artist_key` | `artist_key`, `album_count`, `song_count`, `fan_count`, `updated_at` | 月更 |
...
| QQ v8 五库 | 标签 ID 索引表 | 提供 QQ 侧全量基础 ID 和姓名 | 补齐 QQ-only 艺人 |
| 网易基础库 | 标签 ID 索引表 | 提供网易侧基础 ID 和姓名 | 补齐 网易-only 艺人 |
| 标签 ID 索引表 | 标签表 | 1 对 1 | 让标签表只围绕 `artist_key` 组织 |
| 标签 ID 索引表 | QQ 月更表 | 1 对 1 | 让月更表不必重复存平台 ID |
| 标签 ID 索引表 | 网易月更表 | 1 对 1 | 让月更表不必重复存平台 ID |
...
| 标签表 | 双平台主姓名、性别、地区、语言、风格 | 平台 ID、月更统计、复杂来源说明 |
